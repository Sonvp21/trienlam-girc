...
<head>
  ...
  <style>
    ...
    .text-brand-blue { color: #1E4E8C; } .bg-brand-blue { background-color: #1E4E8C; }
    .border-brand-blue { border-color: #1E4E8C; } .text-brand-red { color: #C0392B; }
  </style>
</head>
    ...
    <div class="relative">
      ...
      <main class="mx-auto max-w-7xl px-4 pb-16 sm:px-6 flex flex-col gap-5">
        @php
          ...
          $sites = [
            ['id'=>'thoi-tiet','name'=>'Thông tin thời tiết và cảnh báo thiên tai','icon'=>'thunderstorm','category'=>'Chính phủ số','url'=>'https://pctt.laocai.gov.vn'],
            ['id'=>'thuy-loi','name'=>'Quản lý hạ tầng các công trình thủy lợi','icon'=>'water_drop','category'=>'Chính phủ số','url'=>'https://thuyloilc.girc.edu.vn'],
            ['id'=>'khoang-san','name'=>'Quản lý khoáng sản','icon'=>'terrain','category'=>'Chính phủ số','url'=>'https://qlks.girc.edu.vn'],
            ['id'=>'moi-truong-khong-khi','name'=>'Cảnh báo chất lượng môi trường không khí','icon'=>'airwave','category'=>'Chính phủ số','url'=>'https://aqicement.girc.edu.vn'],
            ['id'=>'dat-dai','name'=>'Quản lý và quy hoạch sử dụng đất','icon'=>'map','category'=>'Chính phủ số','url'=>'https://landuse.girc.edu.vn'],
            ['id'=>'chuyen-doi-so-xa','name'=>'Chuyển đổi số xã','icon'=>'apartment','category'=>'Chính phủ số','url'=>'https://cdsxa.girc.edu.vn'],
            ['id'=>'ocop','name'=>'Quản lý và phát triển sản phẩm OCOP','icon'=>'storefront','category'=>'Kinh tế số','url'=>'https://ocopbentre.girc.edu.vn'],
          ...
          <div class="relative flex items-center overflow-hidden rounded-2xl border border-white/40 bg-white/80 shadow-lg backdrop-blur-lg">
            <div class="flex w-28 shrink-0 items-center justify-center py-3 sm:w-52">
              <div class="aspect-square w-full overflow-hidden rounded-xl">
                <img src="{{ $cat['image'] }}" alt="{{ $cat['name'] }}" class="h-full w-full object-cover" onerror="this.style.display='none'" />
              </div>
            </div>
            <ul class="min-w-0 flex-1 divide-y divide-slate-100 px-2 py-2 sm:px-3">
              @foreach ($items as $s)
              <li>
                <a href="{{ $s['url'] }}" target="_blank" rel="noopener noreferrer" class="flex w-full items-start gap-2.5 rounded-lg px-2.5 py-2.5 text-left transition hover:bg-slate-50">
                  <span class="material-symbols-outlined mt-0.5 shrink-0 text-lg {{ $cat['accent'] }}">{{ $s['icon'] }}</span>
                  <span class="min-w-0 flex-1">
                    <span class="block text-sm font-medium text-slate-700 sm:text-[15px]">{{ $s['name'] }}</span>
                    <span class="mt-0.5 block truncate text-xs text-slate-400">{{ $s['url'] }}</span>
                    @if (!empty($s['credentials']))
                    <span class="mt-1 flex flex-wrap gap-x-3 gap-y-0.5">
                      @foreach ($s['credentials'] as $c)
                      <span class="text-xs text-slate-500"><span class="font-medium">{{ $c['label'] }}:</span> {{ $c['value'] }}</span>
                      @endforeach
                    </span>
                    @endif
                  </span>
                  <span class="material-symbols-outlined mt-0.5 shrink-0 text-base text-slate-400">open_in_new</span>
                </a>
              </li>
              @endforeach
            </ul>
